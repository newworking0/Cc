...
def extract_multiple_cards(text):
    lines = text.strip().splitlines()[1:]  # skip /mass command line
    cards = []
    for line in lines:
...
def check_card(number, exp_month, exp_year, cvc):
    ...
    try:
        # Stripe test charge - amount is 10 USD cents, currency USD
...
def generate_cards(message):
    ...
    if len(parts) < 2:
        bot.reply_to(message, "Please provide BIN. Example: /gen 446542")
        return
...
def mass_check(message):
    ...
    for i, card in enumerate(cards, 1):
        ...
        time.sleep(1.5)  # Small delay to avoid API rate limits
